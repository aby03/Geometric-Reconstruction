...
class Net:
    ...
    def __init__(self, graph_fp, labels_fp, num_classes=9, threshold=0.6):
    ...
    def thread_display(self, display_img):
    ...
    def predict(self, img, display_img):
        ...
        with self.graph.as_default():
            ...
            num_detections = num_detections.astype(np.int64)
            filtered_results = []
            p_class = []
            p_score = []
            for i in range(0, num_detections[0]):
                score = scores[0][i]
            ...
            print('Elapsed Time = ', elapse, 'ms')
            print('Session Time = ', session_elapse, 'ms')
            d_img = self._display(filtered_results, processed_img=img_copy, display_img=display_img)
        ...
        # del session
        self.in_progress = False
        #print('Returning from net')
        return [p_class, p_score, d_img]
